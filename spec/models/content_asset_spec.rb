...
RSpec.describe ContentAsset, type: :model do
  ...
  describe "antivirus check" do
    let(:content_asset) { ContentAsset.new(title: "Title", alt_text: "Alternative Title") }
    it "adds correct error translation" do
      skip("TODO clamav")
      content_asset.asset_file.attach(io: File.open("spec/fixtures/dirty.pdf"), filename: "dirty.pdf", content_type: "application/pdf")
      content_asset.validate
      expect(content_asset.errors[:asset_file]).to contain_exactly("contains a virus")
    end
    ...
    it "does not store the file" do
      ...
      content_asset.asset_file.attach(io: File.open("spec/fixtures/dirty.pdf"), filename: "dirty.pdf", content_type: "application/pdf")
      content_asset.validate
      expect(content_asset.asset_file).to_not be_persisted
    end
  end
